...
export default function PublicHome() {
  return <div className="min-h-screen bg-[#f8fafc] text-slate-950">
    ...
    <main>
      ...
      <section id="product" className="mx-auto max-w-7xl px-5 py-20 lg:px-8"><div className="max-w-2xl"><p className="text-sm font-bold uppercase tracking-[.18em] text-indigo-600">One service loop</p><h2 className="mt-3 text-4xl font-black tracking-tight">Enquire → Book → Deliver → Get paid.</h2><p className="mt-4 text-slate-600">The CRM, inbox, booking engine, field work, payments and safe automations share one customer and service record.</p></div><div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{featureCards.map(({icon:Icon,title,body})=><div key={title} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm"><div className="mb-4 flex h-10 w-10 items-center justify-center rounded-xl bg-slate-950 text-white"><Icon className="h-5 w-5"/></div><h3 className="font-bold">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-600">{body}</p></div>)}</div></section>
